document.addEventListener('DOMContentLoaded', function () {
      ...
      if (field.hasAttribute('required') && value === '') {
        ...
        if (type === 'email') {
          ...
          if (field.hasAttribute('maxlength')) {
            ...
            if (value.length > max) {
              errorMessage = 'Please enter no more than ' + max + ' characters.';
            }
          }
        }
      }
  ...
  function showFieldError(field, message) {
    ...
    const existingError = getErrorElement(field);
  ...
  function initScrollAnimations() {
    const elements = document.querySelectorAll(
      ...
      '.team-member, .blog-card, .service-item, .gallery-item'
    );
      ...
      mutation.addedNodes.forEach(function (node) {
        if (node.nodeType === 1) {
          ...
          animatableSelectors.forEach(function (selector) {
            if (node.matches && node.matches(selector) && !node.classList.contains(ANIMATION_CLASS)) {
              initScrollAnimations();
            }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Boxpace Furniture משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: 'd1cdd286-f27c-4a1f-8981-a13487517105',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || 'd1cdd286-f27c-4a1f-8981-a13487517105';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'd1cdd286-f27c-4a1f-8981-a13487517105';
